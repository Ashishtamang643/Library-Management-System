...
excel_file = './final_data.csv'  # Replace with your file path (CSV file)
...
for index, row in df.iterrows():
    ...
    insert_query = """
        INSERT INTO books (book_id, book_num, book_name, author_name, available_quantity, 
                           book_edition, publication, faculty, semester, total_quantity)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    """
    
    # Execute the insert query
    cursor.execute(insert_query, (book_id, book_num, book_name, author_name, qty, 
                                  book_edition, publication, faculty, sem, total_qty))
